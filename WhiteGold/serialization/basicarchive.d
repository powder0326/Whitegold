...
public:
	...
	*******************************************************************************/
	void describe(T)(ref T dat, Tracking tracking = Tracking.Default) 
	{
		// resolve typedefs
...
protected:
	/// serialize structs
	void describe_struct(T)(ref T x)
	{
		x.describe(this);
	}
	
	/// serialize classes
	void describe_class(T)(ref T x)
	{
		if (write_or_read == WriteRead.Write)
		{
		...
		else if (write_or_read == WriteRead.Read)
		{
			...
			if (!(name in class_register.by_name))
			{
				...
				{
					throw new Exception("Trying to read class that has been written with the " ~ cast(string)name ~ " tag " ~
	...
	void describe_array(T)(ref T[] x, Tracking tracking = Tracking.Default)
	{
		uint len;
		if (write_or_read == WriteRead.Read)
		{
	...
	/// serialize pointers
	void describe_pointer(T)(ref T* x)
	{
