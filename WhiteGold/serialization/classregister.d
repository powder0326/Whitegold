...
template TClassRegisterPublicInterface(AR)
{
	...
	static void registerClass(T)(string name = T.mangleof)
	{
		if(name in ClassRegister!(AR).by_name)
			throw new Exception("There's already a class registered with the name " ~ name);
		
		auto entry = new ClassRegister!(AR).ClassDescriptionDefault!(T)(cast(char[])name);
	...
	static void registerClassConstructor(T)(T delegate() constructor, string name = T.mangleof)
	{
		if(name in ClassRegister!(AR).by_name)
			throw new Exception("There's already a class registered with the name " ~ name);
		
		auto entry = new ClassRegister!(AR).ClassDescription!(T)(cast(char[])name, constructor);
		ClassRegister!(AR).by_name[name] = entry;
		ClassRegister!(AR).by_classinfo[T.classinfo] = entry;
	}
}
